...
<script>
  ...
  let animId;
  function startAnim() {
    cancelAnimationFrame(animId); // Garante que apenas uma animação esteja rodando
    function step() {
      offset -= 0.35; // Velocidade de rolagem
      if (noticias.length > 0) {
        // Largura total de um card com a nova estrutura (ajuste se mudar padding/margin do card-noticia)
        const singleCardTotalWidth = 350 + 20; // min-width + margin-right (para desktop)
        const totalContentWidth = singleCardTotalWidth * (noticias.length / 2); // Largura de um conjunto de notícias
  ...
  onMount(async () => {
    ...
    if (typeof window !== 'undefined') {
        updateInterval = setInterval(fetchNoticias, 3 * 60 * 1000);
    }
  });
...
<style>
  ...
  .rotativa {
    ...
    left: 0;
    z-index: 200;
    height: 100px !important; /* Altura padrão para desktop */
  ...
  .card-noticia {
    ...
    border-radius: 16px;
    padding: 16px;
    min-width: 350px; /* Largura total do card (200px imagem + gap + 200px texto + padding) */
    max-width: 500px; /* Um pouco mais de flexibilidade */
    height: 100px; /* Ajustado para acomodar a nova altura dos elementos */
    margin-right: 20px;
  ...
  .titulo {
    font-size: 1.0rem; /* Ajuste o tamanho da fonte para caber no espaço */
    font-weight: 600;
    line-height: 1.3;
    color: #fff;
    display: -webkit-box;
  ...
  @media (max-width: 768px) {
    ...
    .card-noticia {
      min-width: 280px; /* Ajuste a largura do card para telas menores */
      max-width: 320px;
      height: 90px;
      padding: 12px;
      margin-right: 15px;
